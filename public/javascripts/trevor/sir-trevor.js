...
(function ($, _){
  ...
  */
  ...
  SirTrevor.editorStore = function(editor, method, options) {
    ...
    switch(method) {
      ...
      case "save":
    	// It is not the sir-trevor method
    	SirTrevor.addImagesInfo();
  ...
  // add previous image id and position to the json data 
  SirTrevor.addImagesInfo = function(){
	  $('.rpt-img').each(function(){
		  var imgId  = $(this).attr('class').split(' ')[4].replace('rpt-img-', '');
		  var pos = $(this).index() - 2;
		  var imgData = "{\"type\": \"image\", \"data\": {\"id\":\"" + imgId + "\", \"position\":\"" + pos + "\"}}";
		  editor.dataStore.data.push($.parseJSON(imgData));
	  });
  };
  ...
  */
  ...
  SirTrevor.fileUploader = function(block, file, success, error) {
    ...
    // commtent the Sir-Trevor default one
    /*data.append('attachment[name]', file.name);
    data.append('attachment[file]', file);
    data.append('attachment[uid]', uid);*/
    
    data.append('resource.fileName', file.name);
    ...
    var callbackSuccess = function(data){
      ...
      if (!_.isUndefined(success) && _.isFunction(success)) {
    	  $('#' + block.blockID).addClass("rpt-img rpt-img-" + data); //add id to image block
  ...
  SirTrevor.bindFormSubmit = function(form) {
    if (!formBound) {
      ...
      form.submit(function(e){
    	  ...
    	  $.ajax({
              url: "/classlifecontroller/createReport",
              type: "POST",
              data: $(form).serialize(),
